const newFormHandler = async (event) => {
  ...
  //get all values from the form
    ...
    const location_form_value = document.querySelector('#location-name').value.trim();
  ...
  //send all values to the server
    if (trip_name_form_value && location_form_value && starting_date_form_value && ending_date_form_value) {
      const response = await fetch(`/api/trip`, {
        method: 'POST',
        body: JSON.stringify({ trip_name_form_value, location_form_value, starting_date_form_value, ending_date_form_value, gear_form_value}),
        headers: {
